...
const TemplatePreview = ({ id }) => {
  ...
  if (id === 'noir') {
    return (
      <div className="relative h-14 w-20 rounded-lg border border-black bg-black">
        <div className="absolute inset-2 rounded-sm bg-slate-900"></div>
        <div className="absolute inset-3 rounded-sm border border-white/30"></div>
      </div>
    );
  }
  if (id === 'airy') {
  ...
  if (id === 'borderline') {
    return (
      <div className="relative h-14 w-20 rounded-lg border border-slate-200 bg-white">
        <div className="absolute inset-2 rounded-sm border border-slate-300"></div>
        <div className="absolute inset-3 rounded-sm border border-slate-200"></div>
      </div>
    );
  }
  if (id === 'editorial') {
    return (
      <div className="relative h-14 w-20 rounded-lg border border-slate-200 bg-white">
        <div className="absolute inset-2 rounded-sm bg-slate-100"></div>
        <div className="absolute bottom-2 left-2 right-6 h-1.5 rounded-full bg-slate-500"></div>
      </div>
    );
  }
  if (id === 'museum') {
    return (
      <div className="relative h-14 w-20 rounded-lg border-4 border-stone-300 bg-stone-200">
        <div className="absolute inset-2 rounded-sm border border-stone-300 bg-white"></div>
        <div className="absolute bottom-2 left-4 right-4 h-2 rounded bg-stone-300"></div>
      </div>
    );
  }
  return (
...
export default function ControlPanel({
  ...
  const currentTemplate = TEMPLATES.find((template) => template.id === config.template) || TEMPLATES[0];

  return (
    <aside className="flex h-screen w-full flex-col border-r border-slate-200/70 bg-white/80 backdrop-blur-xl shadow-[0_20px_60px_-40px_rgba(15,23,42,0.5)] dark:border-slate-800 dark:bg-slate-950/70 md:w-[360px]">
      <div className="border-b border-slate-200/70 px-6 py-5 dark:border-slate-800">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">摄影水印边框工作台</div>
          </div>
        </div>
      </div>
